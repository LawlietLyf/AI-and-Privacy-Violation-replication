...
CONFIG = {
    ...
    'max_retries': 2,
    'retry_delay': 10,
    'input_dir': str(Path(__file__).resolve().parents[1] / "outputs" / "processing_renamed"),
    'save_dir': str(Path(__file__).resolve().parents[1] / "outputs" / "processing_renamed"),
    'temp_dir': str(Path(__file__).resolve().parents[1] / "outputs" / "processing_renamed"),
    'today': "2023-10-30",
    'small_sample': 1,
    'sample_size': 2000,
}
...
def main():
    ...
    first_round_scores.to_csv(os.path.join(first_round_dir, f'scores_nosense_{model}.csv'), index=False)
...
def generate_random_sample(n):
    ...
    sample_df['buy_AIDS_related_product'] = sample_df['XXX'].apply(
...
def predict_buy_AIDS_related_product(df, summary_stats):
    ...
    **Output Format**:
    ...
    2. Ensure prediction fairness and avoid any discrimination. 
