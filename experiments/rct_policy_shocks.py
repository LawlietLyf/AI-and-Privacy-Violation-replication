...
def base_rct_system(shock: str) -> str:
    ...
    extras = [
        f"For the RCT, the pre-shock stated penalty for a detected privacy violation is ${200 if shock == 'penalty' else 1000:,}."
    ]
    if shock == "detection":
...
def score_stage(client, model: str, messages: list[dict], users: pd.DataFrame, temperature: float):
    scores, raw_responses, stage_messages = [], [], copy.deepcopy(messages)
    ...
    out = users.copy()
    out["score"] = scores
    out["raw_response"] = raw_responses
...
def main() -> None:
    ...
    p.add_argument("--pairs", type=int, required=True, help="Set the number of matched pairs explicitly.")
    p.add_argument("--users-per-stage", type=int, required=True, help="Set the number of users per period explicitly.")
